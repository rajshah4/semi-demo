...
5. If the request is RTL, Verilog, SystemVerilog, VHDL, or hardware design work,
   ...
   conversations through Conversation v1, passes the approved `HF_TOKEN` to the
   RTL specialist child as a create-time child secret, waits for child finals,
   and writes a lifecycle report.
...
- create-time Conversation v1 `secrets` for `HF_TOKEN`, only for the RTL
  specialist child
